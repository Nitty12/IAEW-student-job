...
class ExtendedKalmanFilter(object):
    ...
    def update(self, z, HJacobian, Hx, R=None, args=(), hx_args=(),
        ...
        try:
            logging.info('trying normal inverse')
            self.K = PHT.dot(linalg.inv(self.S))
        except np.linalg.LinAlgError as err:
            if 'Singular matrix' in str(err):
                logging.warning('Singular matrix - except block in EKF')
                returnVal = -1
        #         try:
        #             logging.info('trying pseudo inverse')
        #             self.K = PHT.dot(linalg.pinv(self.S))
        #         except np.linalg.LinAlgError as err:
        #             if 'SVD did not converge' in str(err):
        #                 print("Inverse and pseudo inverse not working!")

        hx = Hx(self.x_old, *hx_args)
        self.y = residual(z, hx)
        self.x_new = self.x_old + dot(self.K, self.y)
