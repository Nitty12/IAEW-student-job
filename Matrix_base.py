...
class BaseAlgebra:
    ...
    def _e2v(self, E):
        # edited by Nitty
        # in our case the E has also values for the slack bus
        self.v = E[self.num_non_slack_bus + 1:]
        self.delta = E[:self.num_non_slack_bus + 1]
    ...
    def create_hx_jacobian(self, E):
        ...
        return np.r_[s_jac,
                     v_jac,
                     i_jac][self.non_nan_meas_mask, :]

    def create_inp_jacobian(self, E):
        # Using sparse matrix in creation sub-jacobian matrix
        v, delta = self._e2v(E)
